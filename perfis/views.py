...
def aceitar(request, *args, **kwargs):
    ...
    try:
        ...
        raise exceptions.NotFound(f'Não foi encontrado um convite com o id {kwargs['convite_id']} informado.')
    
    convite.aceitar()
    return response.Responde({'mensagem': 'Convite aceito com sucesso'},
                            status = status.HTTP_201_CREATED)
...
def index(request):
    ...
    print(request.user.has_perm(redirect))
